...
public class SimulationManager implements Runnable {
	...
	private void generateNRandomTasks(int n)
	{
		...
		for(int i = 0; i < n; i++)
		{
			...
			int procTime = rand.nextInt(maxProcessingTime - minProcessingTime) + minProcessingTime;
	...
	public void run() {
		...
		while(currentTime < 3 * timeLimit / 2)
		{
			currentTime++;
			...
			if(currentTime == 3 * timeLimit / 2) frame.writeData(scheduler.dateAsteptare());
		}
	}
